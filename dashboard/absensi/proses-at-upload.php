...
if(isset($_SESSION['user']) && $level >=1 && $level <=8){
    ...
    //    echo $_POST['att_type_upload'];
    $query = mysqli_query($link, "SELECT `root` FROM external_directory WHERE keterangan = 'ATT' ")or die(mysqli_error($link));
    $sql = mysqli_fetch_assoc($query);
    $root_path = $sql['root'];
        $date = $_POST['tanggal_kerja_ot'];
        $temp_path = $_FILES['file_ot']['tmp_name'];
        $ext = pathinfo($_FILES['file_ot']['name'], PATHINFO_EXTENSION);
        // $fileName = $_POST['att_type_upload']."-".$_POST['name_requester']."_".$_POST['group_ot_name']."-".$_POST['shift_ot']."_".$_POST['tanggal_kerja_ot'].".$ext";
        $fileName = $_POST['att_type_upload']."-".$_POST['att_npk']."_".$_POST['group_ot_name']."-".$_POST['shift_ot']."_".$_POST['tanggal_kerja_ot'].".$ext";
        // echo $fileName;
    ...
    //    echo $pathTujuan ;
       if(mysqli_num_rows($query)>0){
                ...
                if(file_exists($pathTujuan1)){
                    ...
                    <script>
                        Swal.fire({
                            ...
                            text: 'Dokumen Pengajuan di Berhasil Dikirim',
                            timer: 2000,
        ...
        <script>
            Swal.fire({
                title: 'Data Kosong',
                text: 'Pastikan Dokumen Pengajuan Telah Dipilih',
                timer: 2000,
