...
if(isset($_GET['value']) || isset($_GET['parent']) ){
    ...
    if(mysqli_num_rows($q_data)>0){
        ?>
        <option value="-" <?=$dis_dept_acc?> >-</option>
        <?php
        while($data = mysqli_fetch_assoc($q_data)){
            ...
            ?>
            <option <?=$selected?> <?=$dis_dept_acc?> value="<?=$data['id']?>"><?=$data['nama_org']?> - <?=$data['nama_cord']?></option>
            <?php
        }
    ?>
    ...
    <?php
        ...
        ?>
        <option value="-" <?=$dis_dept_acc?> ><?=$_GET['parent']?></option>
        <?php
    }
}else{
