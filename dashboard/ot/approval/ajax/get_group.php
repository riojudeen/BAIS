        ...
        if($access_org == 'grp'){
            ...
            if(mysqli_num_rows($sql)>0){
                while($data = mysqli_fetch_assoc($sql)){
                    $query_org = mysqli_query($link, "SELECT nama_org, id FROM view_daftar_area WHERE id = '$data[data]' AND part = 'group' ")or die(mysqli_error($link));
            ...
            if($access_org == 'dept' || $access_org == 'division' || $access_org == 'sect'){
                ...
                $query_org = mysqli_query($link, "SELECT nama_org, id FROM view_daftar_area WHERE id = '$group' AND part = 'group' ")or die(mysqli_error($link));
            }
