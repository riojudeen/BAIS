...
if(isset($_SESSION['user'])){
    if($level >=1 && $level <=8){
        ...
        $exception = " AND `status` <> '100a' AND req_date IS NOT NULL ";
        ...
        if($access_org == 'dept'){
            ...
            if(mysqli_num_rows($sql)>0){
                while($data = mysqli_fetch_assoc($sql)){
                    $query_org = mysqli_query($link, "SELECT nama_org, id FROM view_daftar_area WHERE id = '$data[data]' AND part = 'dept' ")or die(mysqli_error($link));
            ...
            if($access_org == 'division'){
                $query_org = mysqli_query($link, "SELECT nama_org, id FROM view_daftar_area WHERE part = 'dept' AND id_parent = '$_GET[data]' ")or die(mysqli_error($link));
            }else{
                $query_org = mysqli_query($link, "SELECT nama_org, id FROM view_daftar_area WHERE id = '$dept' AND part = 'dept' ")or die(mysqli_error($link));
            }
